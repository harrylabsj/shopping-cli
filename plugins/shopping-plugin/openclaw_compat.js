...
export function resolveShoppingPluginConfig(api, pluginId = OPENCLAW_PLUGIN_ID) {
  ...
  return {
    ...
    pythonPath: nonEmptyString(cfg.pythonPath)
      || nonEmptyString(process.env.SHOPPING_PYTHON)
      || 'python3',
    writesEnabled,
  };
}

export function buildShoppingCommand({ subcommandArgs = [], dataPath, projectRoot, pythonPath } = {}) {
  const root = resolveProjectRoot(projectRoot);
  const command = [nonEmptyString(pythonPath) || nonEmptyString(process.env.SHOPPING_PYTHON) || 'python3', path.join(root, 'scripts', 'shopping.py')];
  if (dataPath) command.push('--db', String(dataPath));
  command.push(...subcommandArgs.map(String));
  return command;
}

export async function runShoppingCli({ subcommandArgs = [], dataPath, projectRoot, pythonPath } = {}) {
  const command = buildShoppingCommand({ subcommandArgs, dataPath, projectRoot, pythonPath });
...
function runConfiguredShoppingCli(config, subcommandArgs) {
  return runShoppingCli({
    subcommandArgs,
...
export function registerOpenClawPlugin(api) {
  ...
  if (registrationConfig.writesEnabled) {
    registerTool(api, toolSpec(api, {
      ...
      return runConfiguredShoppingCli(config, args);
    }));
  ...
  }, async (input, config) => runConfiguredShoppingCli(
    config,
    ['buyer', 'summarize', '--conversation', input.conversation, '--format', 'json'],
  )));
  ...
  if (registrationConfig.writesEnabled) {
    registerTool(api, toolSpec(api, {
      ...
      config,
      ['buyer', 'intent', '--conversation', input.conversation, '--intent', input.intent, '--text', input.text, '--format', 'json'],
    )));
    ...
    registerTool(api, toolSpec(api, {
      ...
      config,
      ['agent', 'run', '--merchant', input.merchant, '--once', '--format', 'json'],
    )));
  }
  ...
  registerCommand(api, {
    ...
    handler: async (ctx = {}) => {
      ...
      if (rawArgs.startsWith('search ')) {
        ...
        const payload = await runConfiguredShoppingCli(
          config,
          ['search', 'products', '--query', query, '--format', 'json'],
        );
